...
describe("Yearly Recurrence -> weekdayInYear", () => {
    const currentInfo =
        YEAR_RECURRENCE_INFO.find(
            (info) =>
                info.recurrenceType === MonthYearRecurrenceType.weekdayInYear
    ...
    describe("getProps", () => {
        it("should put the weekdayInYear and weekday in the options", () => {
            const result = currentInfo.getProps(dateStats);

            expect(result.bysetpos).toMatchObject([dateStats.weekdayInYear]);
    ...
    describe("getDisplay", () => {
        describe("when called with date stats", () => {
            it("should return the expected text", () => {
                ...
                expect(result).toBe("on the 37th Tuesday of the year");
            });
        });
    });
});
