...
describe("Yearly Recurrence -> weekdayInMonth", () => {
	const currentInfo = YEAR_RECURRENCE_INFO.find((info) => info.recurrenceType === MonthYearRecurrenceType.weekdayInMonth) ?? YEAR_RECURRENCE_INFO[0];
	...
	describe("getProps", () => {
		it("should put the weekdayInMonth, weekday and month in the options", () => {
			...
			expect(result.bymonth).toMatchObject([
				dateStats.month,
			]);
		});
	});
