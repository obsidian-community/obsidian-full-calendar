...
describe("Yearly Recurrence -> weekdayInMonth", () => {
    const currentInfo =
        YEAR_RECURRENCE_INFO.find(
            (info) =>
                info.recurrenceType === MonthYearRecurrenceType.weekdayInMonth
    ...
    describe("getProps", () => {
        it("should put the weekdayInMonth, weekday and month in the options", () => {
            const result = currentInfo.getProps(dateStats);

            expect(result.bysetpos).toMatchObject([dateStats.weekdayInMonth]);
    ...
    describe("getDisplay", () => {
        describe("when called with date stats", () => {
            it("should return the expected text", () => {
                ...
                expect(result).toBe("on the 2nd Tuesday of September");
            });
        });
    });
});
