...
import { MONTH_RECURRENCE_INFO, MonthYearRecurrenceType, getDateStats } from "../event-recurrence-types";
import { RRule } from "rrule";
import { defaultOptions } from "./test-helpers";

describe("Monthly Recurrence -> weekdayBeforeEndOfMonth", () => {
	const currentInfo = MONTH_RECURRENCE_INFO.find((info) => info.recurrenceType === MonthYearRecurrenceType.weekdayBeforeEndOfMonth) ?? MONTH_RECURRENCE_INFO[0];
	...
	describe("getProps", () => {
		it("should put the weekdaysFromMonthEnd and weekday in the options", () => {
